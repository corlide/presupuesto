	...
	me.changeView = function(token) {
		...
		if (node !== null) {
			...
			if (node == root || node.parent == root && node.children.length < 2) {
				...
				rad2 = 0 - Math.max(
					hw *0.8 - tgtScale * (a2rad(node.parent.amount)+a2rad(node.amount*1.15 + node.maxChildAmount*1.15)), // maximum visible part
...
BubbleTree.Utils.formatNumber = function(n) {
	...
	if (n >= 1000000000000) return prefix+Math.round(n / 10000000000)/100 + ' billones';
	if (n >= 1000000000) return prefix+Math.round(n / 100000000)/10 + ' mil millones';
...
BubbleTree.Bubbles.Icon = function(node, bubblechart, origin, radius, angle, color) {
	...
	me.show = function() {
		...
		me.label = $('<div class="bubbletree-label '+me.node.id+'"><div class="bubbletree-amount '+me.node.id+'">'+
        me.bc.config.formatValue(me.node.amount)+'</div><div class="bubbletree-desc '+me.node.id+'">'+
        me.node.shortLabel+'</div></div>');
		me.bc.$container.append(me.label);
